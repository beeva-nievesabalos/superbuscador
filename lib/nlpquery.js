...
exports.querySearch = function(query, callback_http){
	...
	child = exec(command + "'"+query+"'", function (error, stdout, stderr) {
		sys.print('stdout: ' + stdout);
		sys.print('stderr: ' + stderr);
		if (error !== null) {
			console.log('exec error: ' + error);
			callback_http(true, "Response: '"+ error +"");
		}
		else {
			// no error:
			// Aquí debería sacar los Verbos y los sustantivos
			callback_http(false, "Response: '"+stdout +"");
		}
	});
...
exports.query = function(request, response){
	...
	if (query != ''){
		...
		exports.querySearch(querysinacentos, function(error, info){
			if(error == true){
				console.log("\n Error!? " + error);
				//response.send(500, info);
				response.render('index', { title: 'BBVA', information: info});
			} else {
